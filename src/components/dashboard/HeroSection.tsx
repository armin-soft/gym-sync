...
export const HeroSection = ({ stats, currentTime, trainerProfile }: HeroSectionProps) => {
  ...
  const quickStats = [
    { 
      ...
      color: "from-brand-600/20 to-brand-400/20",
    ...
    { 
      ...
      color: "from-dark-600/20 to-dark-400/20",
  ...
  return (
    <motion.div 
      ...
      className={`relative overflow-hidden rounded-2xl p-6 md:p-8 text-white shadow-2xl min-h-[300px] ${getGradientClass('primary')}`}
    >
      {/* Enhanced animated background elements with brand colors */}
      <div className={`absolute inset-0 ${getGradientClass('accent')} opacity-50`} />
      
      {/* Animated background gradients */}
      <motion.div 
        className="absolute -bottom-20 -left-20 w-60 h-60 blur-3xl rounded-full bg-gold-500/30"
        animate={{ 
          scale: [1, 1.2, 1],
          opacity: [0.3, 0.4, 0.3],
        }}
      ...
      <motion.div 
        className="absolute -top-20 -right-20 w-60 h-60 blur-3xl rounded-full bg-brand-500/30"
        animate={{ 
          scale: [1, 1.3, 1],
          opacity: [0.3, 0.4, 0.3],
        }}
