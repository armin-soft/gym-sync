...
interface ModernLoadingHeaderProps {
  gymName?: string;
}

export const ModernLoadingHeader = ({ gymName }: ModernLoadingHeaderProps) => {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <motion.h1 
          className="text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-white via-purple-200 to-indigo-200 bg-clip-text text-transparent leading-tight"
        ...
        >
          {gymName ? (
            <>
              در حال بارگذاری مدیریت <br />
              <span className="text-purple-300">{gymName}</span>
            </>
          ) : (
      ...
      >
        ...
        <span className="font-medium">سیستم مدیریت حرفه‌ای ورزشی</span>
        <Crown className="w-5 h-5 text-yellow-400" />
      </motion.div>
    </div>
  );
