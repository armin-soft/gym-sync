...
export const StudentsTable = ({
  ...
  // دسته‌بندی شاگردان براساس حرف اول اسم
  const groupedStudents = React.useMemo(() => {
    if (sortedAndFilteredStudents.length === 0) return {};
    
    return sortedAndFilteredStudents.reduce((acc, student) => {
  ...
  return (
    <ScrollArea className="h-[calc(100vh-20rem)] rounded-lg">
      ...
      >
        {Object.entries(groupedStudents).map(([letter, letterStudents]) => (
          <Collapsible key={letter} defaultOpen={true} className="bg-white/80 dark:bg-gray-900/80 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 border border-indigo-100/30 dark:border-indigo-900/30">
            <CollapsibleTrigger className="w-full">
              <div className="bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950/30 dark:to-blue-950/30 p-3 px-4 flex items-center justify-between cursor-pointer hover:bg-indigo-100/50 dark:hover:bg-indigo-900/50 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="size-9 rounded-full bg-gradient-to-br from-indigo-500 to-blue-500 text-white flex items-center justify-center font-bold text-lg shadow-md">
                    {letter}
                  </div>
                  <h3 className="font-semibold text-indigo-950 dark:text-indigo-100">
                    شاگردان با حرف {letter}
                  </h3>
                  <Badge variant="outline" className="bg-indigo-100/50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 border-indigo-200 dark:border-indigo-800">
                    {toPersianNumbers(letterStudents.length)} شاگرد
                  </Badge>
                </div>
              </div>
            </CollapsibleTrigger>
