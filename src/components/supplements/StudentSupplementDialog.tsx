...
export function StudentSupplementDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      console.log("Initial supplements:", initialSupplements);
      console.log("Initial vitamins:", initialVitamins);
    }
  ...
  useEffect(() => {
    try {
      ...
      if (propSupplements && propSupplements.length > 0) {
        ...
        console.log("Loaded supplements from props:", propSupplements.length);
      } else {
        ...
        if (savedSupplements) {
          ...
          console.log("Loaded supplements from localStorage:", parsedSupplements.length);
        }
      }
      ...
      if (propCategories && propCategories.length > 0) {
        ...
        console.log("Loaded categories from props:", propCategories.length);
      } else {
        // Fallback to localStorage
  ...
  const toggleItem = (id: number) => {
    if (activeTab === "supplements") {
      ...
      console.log("Toggled supplement:", id);
    } else {
      setSelectedVitamins(prev => 
        prev.includes(id) ? prev.filter(itemId => itemId !== id) : [...prev, id]
      );
      console.log("Toggled vitamin:", id);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[100vw] md:max-w-[100vw] lg:max-w-[100vw] xl:max-w-[100vw] w-[100vw] h-[100vh] max-h-[100vh] p-0 overflow-hidden bg-gradient-to-b from-white to-gray-50 dark:from-gray-900 dark:to-gray-950 border-primary/10 flex flex-col m-0 rounded-none shadow-xl" dir="rtl">
        <DialogTitle className="sr-only">مدیریت مکمل‌ها و ویتامین‌ها</DialogTitle>
        <Tabs 
          value={activeTab} 
        ...
        >
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button 
                        variant="outline" 
                        size="icon" 
                        className={cn("h-9 w-9 border-muted transition-colors", 
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            {/* Footer */}
            <div className="mb-4 p-3 rounded-xl flex flex-wrap gap-2 justify-between items-center bg-white border border-gray-100 shadow-sm border-t p-4 mt-auto bg-muted/20 shrink-0">
              <div className="flex items-center gap-2">
                <motion.div 
                  initial={{scale: 0.9, opacity: 0}}
                  animate={{
                    scale: (activeTab === "supplements" ? selectedSupplements.length > 0 : selectedVitamins.length > 0) ? 1 : 0.9,
                    opacity: (activeTab === "supplements" ? selectedSupplements.length > 0 : selectedVitamins.length > 0) ? 1 : 0
                  }} 
                  className={cn(
                    "bg-gradient-to-r px-3 py-1.5 rounded-full text-xs font-medium text-white flex items-center gap-1.5",
                    activeTab === "supplements" 
                ...
                <motion.div 
                  initial={{scale: 0.9, opacity: 0}}
                  animate={{
                    scale: (activeTab === "supplements" ? selectedVitamins.length > 0 : selectedSupplements.length > 0) ? 1 : 0.9,
                    opacity: (activeTab === "supplements" ? selectedVitamins.length > 0 : selectedSupplements.length > 0) ? 1 : 0
                  }} 
                  className={cn(
                    "bg-gradient-to-r px-3 py-1.5 rounded-full text-xs font-medium text-white flex items-center gap-1.5",
                    activeTab === "supplements" 
                ...
                >
                  ...
                  {toPersianNumbers(
                    activeTab === "supplements" 
                      ? selectedVitamins.length 
                      : selectedSupplements.length
                  )} {activeTab === "supplements" ? "ویتامین" : "مکمل"} انتخاب شده
                </motion.div>
              </div>
