...
export const AdvancedCategoryManager: React.FC<AdvancedCategoryManagerProps> = ({
  ...
  const [categoryDialog, setCategoryDialog] = useState({ open: false, mode: 'add' as 'add' | 'edit', category: null as SupplementCategory | null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, category: null as SupplementCategory | null });
  ...
  const handleEditCategory = (categoryId: number, name: string) => {
    // Logic will be handled by parent component
    setCategoryDialog({ open: false, mode: 'edit', category: null });
  };
  ...
  const openEditDialog = (category: SupplementCategory) => {
    setCategoryDialog({ open: true, mode: 'edit', category });
  };

  const openDeleteDialog = (category: SupplementCategory) => {
  ...
  return (
    <>
      ...
      <CategoryDialog
        ...
        onSubmit={categoryDialog.mode === 'add' ? handleAddCategory : (name) => handleEditCategory(categoryDialog.category?.id || 0, name)}
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog({ ...deleteDialog, open })}>
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              آیا از حذف دسته‌بندی "{deleteDialog.category?.name}" اطمینان دارید؟
              این عمل قابل بازگشت نیست و تمام مکمل‌ها یا ویتامین‌های این دسته حذف خواهند شد.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
